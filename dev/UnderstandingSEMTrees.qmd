...
flowchart TB
    ...
    G -- FALSE --> G2["Assign NULL to node$model w/ error msg"]
    ...
    direction TB
    J["Use getGlobal() set node_id based on environment name"]
    J --> K["Split using one of <br> naiveSplit(), ScoreSplit(), <br> fairSplit(control$fair3Step = <br> TRUE/FALSE)"]
    K --> K1["naiveSplit()"]
    K --> K2["ScoreSplit()"] 
    K --> K3["fairSplit()"]
...
```{mermaid}
flowchart TB
    A["Create lavaan command for node$model"] 
    B["Fit model of 
    node$model using 
    safeRunAndEvaluate()"]
    A --> B
    C[Assign meta$covariate.ids
     to comparedData]
    B --> C
```
...
-    Investigate what `safeRunAndEvaluate()` is up to?
-    Is `node$model` always the same root model or does it change recursively?